...
{	
	function createLinkFromNode(node) {
		...
		otrsTagExpression = new RegExp("(" + regex + ")", "g");
		
		while ( (m=otrsTagExpression.exec(txt)) !== null)
		{
			if (null===span) {
			...
			a.setAttribute('href', otrs_path + link);
	...
	if (options.whitelist.length > 0) {
		for (var i = 0; i < options.whitelist.length; i++) {
			...
			if (curResult == true) {
				...
				// Format otrs path with a trailing slash if not present
				otrs_path = options.otrs_path;
				// if (otrs_path.substr(-1) != '/') otrs_path += '/';
